...
class readlog:
    ...
    def Input(self):
        ...
        with open("D:/02_Program/Log_Analysis/invo-log.txt", mode="r", errors="ignore") as op:
            while True:
                # 每一行提取出来的数据
                a = op.readline()
                ...
                for i in range(int((len(one_a) - 1) / 2)):
                    ...
                    if Cup_00:
                        self.Cup.append(Cup_00)
                    # 这是第二个 因为定义的为列表嵌套，所以就算各个列表为空他也是为1，所以需要判断是否为各个列表为空 返回值为
                    if Fps_00 != [[], [], [], []]:
                        for x in range(0, 4):
                            if Fps_00[x]:
                                self.Fps[x].append(Fps_00[x])
                    #     这是第三个
                    if Sqi_00:
                        self.Sqi.append(Sqi_00)
                    # 这是第四个
                    if MEMOPY_00:
                        self.MEMOPY.append(MEMOPY_00)
                if a == "":
                    break
